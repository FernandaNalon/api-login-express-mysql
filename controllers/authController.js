...
async function cadastrarUsuario(req, res) {
  try {
    ...
    // Se o e-mail já existir, redireciona para cadastro com mensagem de erro
...
async function realizarLogin(req, res) {
  try {
    ...
    // Busca um usuário no banco com o e-mail e senha informados
    const [usuarios] = await conexao.query(
      "SELECT * FROM usuarios WHERE email = ? AND senha = ?",
      [email, senha]
    );

    // Se não encontrar usuário, redireciona com erro
